...
%% Uncomment below to include a teaser figure.
   ...
   \caption{TODO: label subfigs A,B,C,D. A)Aggregated step counts following an intervention event shows overall event response dynamics and individual variations across events. TODO: color by events and make shorter B) Aggregation of step counts showing dramatic response to the the control intervention. C) Heart rate data aggregated across KnowMe particpants shows a mild response to an SMS intervention. D) Aggregated step counts of subjects briefly exposed to an active mAvatar shows little trend. TODO: reference B,C,D from text}
  }
