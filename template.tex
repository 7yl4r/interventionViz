...
where $N$ is the total number of events across all participants and $y_i$ represents the time series for event $i$

\begin{equation}
	\bar{y}(t) = \frac{1}{P} \sum\limits_{p=1}^P \frac{1}{n_p} \sum\limits{i=1}^n_p y_{i,p}(t)
	\label{eq:lineAvg}
\end{equation}
	where $P$ is the number of participants, and $y_{i,p}$ represents the time series for subject $p$'s event $i$, and $n_p$ represents the number of events for subject $p$
